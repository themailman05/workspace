...
const IndexPage = () => {
      ...
      .then(() => {
        ...
        dispatch(
          setSingleActionModal({
            title: 'Success',
            type: 'info',
            content: `You have successfully submitted a transaction to purchase ${formatter.format(
              popToPurchase,
            )} POP! Please wait for the transaction confirmation`,
            onConfirm: {
              label: 'Okay',
              onClick: () => dispatch(setSingleActionModal(false)),
            },
  ...
  const getUSDCAllowance = () => {
    contracts.USDC
    .allowance(account, contracts.privateSale.address)
                      ...
                      <div className="border-t-2 border-gray-100 rounded-b-lg pt-10 pb-8 px-6 bg-gray-50 space-y-4 sm:px-10 sm:py-10">
                        ...
                        <div className="rounded-lg shadow-md">
                          ...
                          {account && allowance > 0 && (
                            <MainActionButton
                              label={`Purchase POP`}
                              handleClick={() => { purchasePop() }}
                              disabled={isTxPending}
                            />
                          )}
                          {account && allowance <= 0 && (
                            <MainActionButton
                              label={`Approve ${purchaseAmountUSDC && formatter.format(
                                purchaseAmountUSDC,
                              ) || ''} USDC`}

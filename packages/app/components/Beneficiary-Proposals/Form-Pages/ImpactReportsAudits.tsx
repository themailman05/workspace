  ...
  files.forEach((file) => {
    ...
    fetch('https://api.pinata.cloud/pinning/pinFileToIPFS', {
      ...
      .then((result) => {
        ...
        const newImpactReports = impactReports.concat(hash);
        setImpactReports(newImpactReports);
      })
    ...
    return (
      <div className="mx-auto content-center justify-items-center">
        ...
        {impactReports?.length > 0 ? (
          <div>
            {impactReports.map((hash) => {
              return <p>{'https://gateway.pinata.cloud/ipfs/' + hash}</p>;
            })}
          </div>
        ) : (
          <> </>

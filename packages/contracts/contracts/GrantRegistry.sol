...
contract GrantRegistry {
    ...
    function getActiveAwardees(uint8 termLength) public view returns (address[] memory) {
      address[] memory ret = new address[](activeGrants[termLength].awardeesCount);
      for (uint i = 0; i < activeGrants[termLength].awardeesCount; i++) {
          ret[i] = activeAwardees[termLength][i].awardee;
      }
      return ret;
    }

    function getActiveGrant(uint8 termLength) public view returns (uint256[] memory grant) {
      uint[] memory _grant = new uint[](5);

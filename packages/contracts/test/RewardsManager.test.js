...
describe('RewardsManager', function () {
  ...
  let MockERC20, Staking, Treasury, BeneficiaryVaults, RewardsManager;
  ...
  beforeEach(async function () {
    ...
    this.mockBeneficiaryVaults = await waffle.deployMockContract(owner, BeneficiaryVaults.interface.format());
  ...
  it("reverts when setting reward splits as non-owner", async function () {
    await expect(
      this.rewardsManager.connect(nonOwner).setRewardSplits([20, 20, 60])
    ).to.be.revertedWith("Only the contract owner may perform this action");
  });
  ...
  describe("reward splits are set", function () {
    beforeEach(async function () {
      newRewardSplits = [parseEther("20"), parseEther("20"), parseEther("60")];
      result = await this.rewardsManager.setRewardSplits(newRewardSplits);
    });

    it("should emit RewardSplitsUpdated event", async function () {
      expect(result).to.emit(this.rewardsManager, "RewardSplitsUpdated").withArgs(newRewardSplits);
    });
  ...
  it("should revert setting to same BeneficiaryVaults", async function () {
    await expect(
      this.rewardsManager.setBeneficiaryVaults(this.mockBeneficiaryVaults.address)
  ...
  describe("sets new dependent contracts", function () {
    ...
    it("sets new BeneficiaryVaults", async function () {
      const newBeneficiaryVaults = await waffle.deployMockContract(owner, BeneficiaryVaults.interface.format());
      result = await this.rewardsManager.setBeneficiaryVaults(newBeneficiaryVaults.address);
      expect(await this.rewardsManager.beneficiaryVaults()).to.equal(newBeneficiaryVaults.address);
      expect(result).to.emit(this.rewardsManager, "BeneficiaryVaultsChanged")
        .withArgs(this.mockBeneficiaryVaults.address, newBeneficiaryVaults.address);
    });
  });
  ...
  describe("send rewards", function () {
    beforeEach(async function () {
      ...
      beneficiaryVaultsReward = firstReward.mul(RewardSplits.BeneficiaryVaults).div(parseEther("100"));
    ...
    describe("rewards are distributed", function () {
      ...
      it("emits expected events", async function () {
        ...
        expect(result).to.emit(this.rewardsManager, "BeneficiaryVaultsDeposited")
          .withArgs(this.mockBeneficiaryVaults.address, beneficiaryVaultsReward);
        expect(result).to.emit(this.rewardsManager, "RewardsDistributed").withArgs(firstReward);
      });
      ...
      it("BeneficiaryVaults has expected balance", async function () {
        expect(await this.mockPop.balanceOf(this.mockBeneficiaryVaults.address)).to.equal(beneficiaryVaultsReward);
      });
      ...
      describe("send more rewards", function () {
        ...
        describe("new rewards are distributed", function () {
          beforeEach(async function () {
            ...
            beneficiaryVaultsSecondReward = secondReward.mul(RewardSplits.BeneficiaryVaults).div(parseEther("100"));
            result = await this.rewardsManager.distributeRewards();
          });
          ...
          it("emits expected events", async function () {
            ...
            expect(result).to.emit(this.rewardsManager, "BeneficiaryVaultsDeposited")
              .withArgs(this.mockBeneficiaryVaults.address, beneficiaryVaultsSecondReward);
            expect(result).to.emit(this.rewardsManager, "RewardsDistributed").withArgs(secondReward);
          });
          ...
          it("BeneficiaryVaults has expected balance", async function () {
            ...
            ).to.equal(beneficiaryVaultsReward.add(beneficiaryVaultsSecondReward));
          });
        });
      });
    });

...
describe("GrantElections", function () {
  before(async function () {
    ...
    this.mockGrantRegistry = await waffle.deployMockContract(
      owner,
      GrantRegistry.interface.format()
    );

    const RandomNumberConsumer = await ethers.getContractFactory(
      "RandomNumberConsumer"
    );
    this.mockRandomNumberConsumer = await waffle.deployMockContract(
      owner,
      RandomNumberConsumer.interface.format()
    );
  ...
  describe("registration", function () {
    ...
    it("should allow beneficiary to register for election with a bond", async function () {
      ...
      const bennies = await this.contract.getRegisteredBeneficiaries(
        GRANT_TERM.YEAR
      );
    ...
    it("should transfer POP to election contract on registration", async function () {
      ...
      const popBalanceForElection = await this.mockPop.balanceOf(
  ...
  describe("getCurrentRanking", function () {
    ...
    it("return current ranking", async function () {
      ...
      expect(
        await this.contract.getCurrentRanking(GRANT_TERM.MONTH)
      ).deep.to.eq([
        beneficiary.address,
  ...
  describe("finalization", function () {
    ...
    it("require election closed", async function () {
      await expect(this.contract.finalize(GRANT_TERM.MONTH)).to.be.revertedWith(
    ...
    it("finalizes an election with randomization", async function () {
      ...
      await this.mockPop
        .connect(beneficiary)
        .approve(this.contract.address, registrationBondQuarter);
      await this.contract
        .connect(beneficiary)

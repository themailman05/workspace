...
describe('BeneficiaryGovernance', function () {
  ...
  before(async function () {
    ...
    this.BeneficiaryGovernance = await BeneficiaryGovernance.deploy(
  ...
  describe("defaults", function () {
    ...
    const defConfig = await this.BeneficiaryGovernance.DefaultConfigurations();
    ...
    await this.BeneficiaryGovernance.connect(owner).setConfiguration(10*ONE_DAY,10*ONE_DAY,parseEther('3000'));
     const defConfig = await this.BeneficiaryGovernance.DefaultConfigurations();
  ...
  describe("proposals", function () {
    ...
    it("should create BNP proposal with specified attributes", async function() {
      ...
      await this.mockPop .connect(proposer2).approve(this.BeneficiaryGovernance.address, parseEther('3000'));
      await this.BeneficiaryGovernance.connect(proposer2).createProposal(beneficiary.address, ethers.utils.formatBytes32String("testCid"),ProposalType.BNP);
      const proposal=await this.BeneficiaryGovernance.proposals(PROPOSALID);
      ...
      expect(await this.BeneficiaryGovernance.getNumberOfProposals()).to.equal(1);
    });
    it("should prevent to create proposal with not enough bond", async function () {
      
      await this.mockPop.connect(proposer1).approve(this.BeneficiaryGovernance.address, parseEther('1500'));
      await expect(
        this.BeneficiaryGovernance.connect(proposer1).createProposal(beneficiary.address, ethers.utils.formatBytes32String("testCid"),ProposalType.BNP)
    ...
    it("should prevent to create a BNP proposal for a pending beneficiary proposal", async function () {
      ...
      await this.mockPop.connect(proposer3).approve(this.BeneficiaryGovernance.address, parseEther('3000'));
      await expect(
        this.BeneficiaryGovernance.connect(proposer3).createProposal(beneficiary.address, ethers.utils.formatBytes32String("testCid"),ProposalType.BNP)
    ...
    it("should prevent to create a BTP proposal for an address which hasn't been registered before", async function () {
      ...
      await this.mockPop.connect(proposer3).approve(this.BeneficiaryGovernance.address, parseEther('3000'));
      await expect(
        this.BeneficiaryGovernance.connect(proposer3).createProposal(beneficiary2.address, ethers.utils.formatBytes32String("testCid"),ProposalType.BTP)
    ...
    it("should prevent to create a BNP proposal for an address which has been registered before", async function () {
      ...
      await this.mockPop.connect(proposer3).approve(this.BeneficiaryGovernance.address, parseEther('3000'));
      await expect(
        this.BeneficiaryGovernance.connect(proposer3).createProposal(beneficiary2.address, ethers.utils.formatBytes32String("testCid"),ProposalType.BNP)
  ...
  describe("voting", function () {
    beforeEach(async function () {
      ...
      this.BeneficiaryGovernance = await BeneficiaryNomination.deploy(
        this.mockStaking.address,
        this.mockBeneficiaryRegistry.address,
        this.mockPop.address,
        owner.address
      );
      await this.BeneficiaryGovernance.deployed();
      // create a BNP proposal
      await this.mockBeneficiaryRegistry.mock.beneficiaryExists.returns(false);
      await this.mockPop.connect(proposer1).approve(this.BeneficiaryGovernance.address, parseEther('2000'));
      await this.BeneficiaryGovernance.connect(proposer1).createProposal(beneficiary.address, ethers.utils.formatBytes32String("testCid"),ProposalType.BNP);
      // create a BTP
      await this.mockBeneficiaryRegistry.mock.beneficiaryExists.returns(true);
      await this.mockPop.connect(proposer2).approve(this.BeneficiaryGovernance.address, parseEther('2000'));
      await this.BeneficiaryGovernance.connect(proposer2).createProposal(beneficiary.address, ethers.utils.formatBytes32String("testCid"),ProposalType.BTP);
    });
    it("should prevent voting without voiceCredits", async function() {
      await this.mockStaking.mock.getVoiceCredits.returns(0);
      await expect(this.BeneficiaryGovernance.connect(voter1).vote(PROPOSALID,Vote.Yes)).to.be.revertedWith( "must have voice credits from staking");
    });
    it("should vote yes to a newly created proposal", async function() {
      ...
      await this.BeneficiaryGovernance.connect(voter1).vote(PROPOSALID,Vote.Yes);
      const proposal=await this.BeneficiaryGovernance.proposals(PROPOSALID);

      expect(proposal.noCount).to.equal(0);
      expect(proposal.voterCount).to.equal(1);
      expect(proposal.yesCount).to.equal(voiceCredits);
      expect(await this.BeneficiaryGovernance.hasVoted(PROPOSALID,voter1.address)).to.equal(true);
    ...
    it("should prevent an address to vote more than one time to a proposal", async function() {
      ...
      await this.BeneficiaryGovernance.connect(voter1).vote(PROPOSALID,Vote.Yes);
      await expect(this.BeneficiaryGovernance.connect(voter1).vote(PROPOSALID,Vote.Yes)).to.be.revertedWith( "address already voted for the proposal");
    });
    it("should prevent to vote yes during veto period", async function() {
      ...
      await expect(this.BeneficiaryGovernance.connect(voter1).vote(PROPOSALID,Vote.Yes)).to.be.revertedWith( "Initial voting period has already finished!");
    });
    it("should prevent to vote after the end of the total voting period", async function() {
      ...
      ethers.provider.send("evm_mine");
      await expect(this.BeneficiaryGovernance.connect(voter1).vote(PROPOSALID,Vote.No)).to.be.revertedWith( "Proposal is no longer in voting period");
    });
    it("should update proposal correctly", async function() {
      ...
      await this.mockStaking.mock.getVoiceCredits.returns(30);
      await this.BeneficiaryGovernance.connect(voter2).vote(PROPOSALID,Vote.Yes);

      //three no votes
      await this.mockStaking.mock.getVoiceCredits.returns(40);
      await this.BeneficiaryGovernance.connect(voter3).vote(PROPOSALID,Vote.No);
      await this.mockStaking.mock.getVoiceCredits.returns(50);
      await this.BeneficiaryGovernance.connect(voter4).vote(PROPOSALID,Vote.No);
      await this.mockStaking.mock.getVoiceCredits.returns(60);
      await this.BeneficiaryGovernance.connect(voter5).vote(PROPOSALID,Vote.No);
    ...
    it("should finalize voting if at the end of the voting perid novotes be more than yesvotes", async function() {
      ...
      //two no votes
      await this.mockStaking.mock.getVoiceCredits.returns(40);
      await this.BeneficiaryGovernance.connect(voter2).vote(PROPOSALID,Vote.No);
      await this.mockStaking.mock.getVoiceCredits.returns(50);
      await this.BeneficiaryGovernance.connect(voter3).vote(PROPOSALID,Vote.No);
      ...
      await this.mockStaking.mock.getVoiceCredits.returns(60);
      await this.BeneficiaryGovernance.connect(voter4).finalize(PROPOSALID);
    ...
    it("should prevent voting if the voting is finalized", async function() {
      ...
      await this.mockStaking.mock.getVoiceCredits.returns(60);
      await expect(this.BeneficiaryGovernance.connect(voter5).vote(PROPOSALID,Vote.No)).to.be.revertedWith( "Proposal is no longer in voting period");
    ...
    it("should countinue voting if at the end of the initial voting yesvotes are more than novotes", async function() {
      ...
      await this.BeneficiaryGovernance.connect(voter2).vote(PROPOSALID,Vote.Yes);
      await this.mockStaking.mock.getVoiceCredits.returns(40);
      await this.BeneficiaryGovernance.connect(voter3).vote(PROPOSALID,Vote.Yes);
      ethers.provider.send("evm_increaseTime", [2 * ONE_DAY]);
      ethers.provider.send("evm_mine");
      //two no votes
      await this.mockStaking.mock.getVoiceCredits.returns(40);
  ...
  describe("finalize", function () {
    ...
    it("should finalize a voting during challenge period if novotes are more than yes votes", async function() {
       ...
       await this.mockStaking.mock.getVoiceCredits.returns(20);
       await this.BeneficiaryGovernance.connect(voter1).vote(PROPOSALID,Vote.No);
       await this.mockStaking.mock.getVoiceCredits.returns(30);
       await this.BeneficiaryGovernance.connect(voter2).vote(PROPOSALID,Vote.No);
      ...
      await this.BeneficiaryGovernance.connect(owner).finalize(PROPOSALID);
         //get proposal info
      const proposal=await this.BeneficiaryGovernance.proposals(PROPOSALID);
      expect(proposal.status).to.equal(ProposalStatus.Failed);
    });
    it("should prevent finalizing  a finalized voting", async function() {
     ...
     await this.BeneficiaryGovernance.connect(owner).finalize(PROPOSALID);
     await expect(this.BeneficiaryGovernance.connect(owner).finalize(PROPOSALID)).to.be.revertedWith("Finalization not allowed");
   });
   ...
   await this.beneficiaryRegistryContract.transferOwnership(this.BeneficiaryGovernance.address);
    ...
    //finalize 
   await this.BeneficiaryGovernance.connect(governance).finalize(PROPOSALID);
 ...
 it("should remove beneficiary after a successful BTP voting", async function() {
  ...
  //await this.beneficiaryRegistryContract.approveOwner(this.BeneficiaryGovernance.address);
  ...
  //three yes votes
  await this.mockStaking.mock.getVoiceCredits.returns(20);
  await this.BeneficiaryGovernance.connect(voter1).vote(PROPOSALID_BTP,Vote.Yes);
  await this.mockStaking.mock.getVoiceCredits.returns(30);
  await this.BeneficiaryGovernance.connect(voter2).vote(PROPOSALID_BTP,Vote.Yes);
  await this.mockStaking.mock.getVoiceCredits.returns(40);
  await this.BeneficiaryGovernance.connect(voter3).vote(PROPOSALID_BTP,Vote.Yes);
  ethers.provider.send("evm_increaseTime", [2 * ONE_DAY]);
  ethers.provider.send("evm_mine");
  //two no votes
  await this.mockStaking.mock.getVoiceCredits.returns(30);
  await this.BeneficiaryGovernance.connect(voter4).vote(PROPOSALID_BTP,Vote.No);
  await this.mockStaking.mock.getVoiceCredits.returns(20);
  await this.BeneficiaryGovernance.connect(voter5).vote(PROPOSALID_BTP,Vote.No);
  ethers.provider.send("evm_increaseTime", [2 * ONE_DAY]);
  ethers.provider.send("evm_mine");
  
  //finalize
 await this.BeneficiaryGovernance.connect(governance).finalize(PROPOSALID_BTP);
...
describe("claimBond", function () {
    ...
    it("should prevent claiming bond whith address other than the proposer address", async function() {
      ...
      await expect(this.BeneficiaryGovernance.connect(owner).claimBond(PROPOSALID)).to.be.revertedWith("only the proposer may call this function");
   });
   it("should prevent claiming bond for a proposal which has not passed.", async function() {
    ...
    await expect(this.BeneficiaryGovernance.connect(proposer1).claimBond(PROPOSALID)).to.be.revertedWith("Proposal failed or is processing!");
 });
    it("should be able to claim bond after a proposal passed.", async function() {
      ...
      expect(await this.BeneficiaryGovernance.connect(proposer1).claimBond(PROPOSALID))
      .to.emit(this.BeneficiaryGovernance, "BondWithdrawn")

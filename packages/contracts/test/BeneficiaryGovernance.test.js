...
describe("BeneficiaryGovernance", function () {
  ...
  describe("defaults", function () {
    it("should set correct proposal defaults", async function () {
      const defConfig =
        await this.BeneficiaryGovernance.DefaultConfigurations();
    ...
    it("should set configuration for proposals", async function () {
      await this.BeneficiaryGovernance.connect(owner).setConfiguration(
        10 * ONE_DAY,
        10 * ONE_DAY,
        parseEther("3000")
      );
  ...
  describe("proposals", function () {
    it("should create BNP proposal with specified attributes", async function () {
      ...
      await this.mockPop
        .connect(proposer2)
        .approve(this.BeneficiaryGovernance.address, parseEther("3000"));
      const result = await this.BeneficiaryGovernance.connect(
        proposer2
      ).createProposal(
        beneficiary.address,
      ...
      expect(await this.BeneficiaryGovernance.getNumberOfProposals()).to.equal(
        1
      );
      expect(result)
        .to.emit(this.BeneficiaryGovernance, "VaultInitialized")
    });
    it("should prevent to create proposal with not enough bond", async function () {
      await this.mockPop
        .connect(proposer1)
        .approve(this.BeneficiaryGovernance.address, parseEther("1500"));
      await expect(
        this.BeneficiaryGovernance.connect(proposer1).createProposal(
          beneficiary.address,
          ethers.utils.formatBytes32String("testCid"),
          ProposalType.BNP
        )
    ...
    it("should prevent to create a BNP proposal for a pending beneficiary proposal", async function () {
      await this.mockPop
        ...
        .approve(this.BeneficiaryGovernance.address, parseEther("3000"));
      await expect(
        this.BeneficiaryGovernance.connect(proposer3).createProposal(
          beneficiary.address,
          ethers.utils.formatBytes32String("testCid"),
          ProposalType.BNP
        )
  ...
  describe("voting", function () {
    beforeEach(async function () {
      ...
      await this.mockPop
        .connect(proposer1)
        .approve(this.BeneficiaryGovernance.address, parseEther("2000"));
      await this.BeneficiaryGovernance.connect(proposer1).createProposal(
        beneficiary.address,
        ethers.utils.formatBytes32String("testCid"),
        ProposalType.BNP
      );
      ...
      await this.mockPop
        .connect(proposer2)
        .approve(this.BeneficiaryGovernance.address, parseEther("2000"));
      await this.BeneficiaryGovernance.connect(proposer2).createProposal(
        beneficiary.address,
        ethers.utils.formatBytes32String("testCid"),
        ProposalType.BTP
      );
    ...
    it("should prevent voting without voiceCredits", async function () {
      ...
      await expect(
        this.BeneficiaryGovernance.connect(voter1).vote(PROPOSALID, Vote.Yes)
    ...
    it("should vote yes to a newly created proposal", async function () {
      ...
      const result = await this.BeneficiaryGovernance.connect(voter1).vote(
        PROPOSALID,
        Vote.Yes
      );
      const proposal = await this.BeneficiaryGovernance.proposals(PROPOSALID);
      ...
      expect(
        await this.BeneficiaryGovernance.hasVoted(PROPOSALID, voter1.address)
      ).to.equal(true);
      expect(result).to.emit(this.BeneficiaryGovernance, "SharesAdded")
    });
    it("should prevent an address to vote more than one time to a proposal", async function () {
      ...
      await this.BeneficiaryGovernance.connect(voter1).vote(
        PROPOSALID,
        Vote.Yes
      );
      await expect(
    ...
    it("should prevent to vote after the end of the total voting period", async function () {
      ...
      ethers.provider.send("evm_mine");
      await expect(
        this.BeneficiaryGovernance.connect(voter1).vote(PROPOSALID, Vote.No)
    ...
    it("should update proposal correctly", async function () {
      ...
      await this.mockStaking.mock.getVoiceCredits.returns(30);
      await this.BeneficiaryGovernance.connect(voter2).vote(
        PROPOSALID,
        Vote.Yes
      );
      ...
      await this.BeneficiaryGovernance.connect(voter3).vote(
        PROPOSALID,
        Vote.No
      );
      await this.mockStaking.mock.getVoiceCredits.returns(50);
      await this.BeneficiaryGovernance.connect(voter4).vote(
        PROPOSALID,
        Vote.No
      );
      await this.mockStaking.mock.getVoiceCredits.returns(60);
      await this.BeneficiaryGovernance.connect(voter5).vote(
        PROPOSALID,
        Vote.No
      );
    ...
    it("should finalize voting if at the end of the voting perid no-votes be more than yes-votes", async function () {
      //one yes vote
      await this.mockStaking.mock.getVoiceCredits.returns(20);
      await this.BeneficiaryGovernance.connect(voter1).vote(
        PROPOSALID,
      ...
      ethers.provider.send("evm_increaseTime", [4 * ONE_DAY]);
      ethers.provider.send("evm_mine");
    ...
    it("should prevent voting if the voting is finalized", async function () {
      ...
      await this.mockStaking.mock.getVoiceCredits.returns(60);
      await expect(
        this.BeneficiaryGovernance.connect(voter5).vote(PROPOSALID, Vote.No)
      ).to.be.revertedWith("Proposal is no longer in voting period");
    });
  ...
  describe("finalize", function () {
    ...
    it("should finalize a voting during challenge period if no-votes are more than yes-votes", async function () {
      await this.mockStaking.mock.getVoiceCredits.returns(20);
      await this.BeneficiaryGovernance.connect(voter1).vote(
        PROPOSALID,
        Vote.No
      ...
      const result = await this.BeneficiaryGovernance.connect(owner).finalize(PROPOSALID);
      //get proposal info
      const proposal = await this.BeneficiaryGovernance.proposals(PROPOSALID);
      expect(proposal.status).to.equal(ProposalStatus.Failed);
      expect(result).to.emit(this.BeneficiaryGovernance, "VaultOpened")
    });
    it("should prevent finalizing a finalized voting", async function () {
      ...
      await expect(
        this.BeneficiaryGovernance.connect(owner).finalize(PROPOSALID)
    ...
    it("should register the beneficiary after a successful BNP voting", async function () {
      await this.beneficiaryRegistryContract.transferOwnership(
        this.BeneficiaryGovernance.address
      );
    ...
    it("should remove beneficiary after a successful BTP voting", async function () {
      ...
      //finalize
      await this.BeneficiaryGovernance.connect(governance).finalize(
        PROPOSALID_BTP
      );
  ...
  describe("claimBond", function () {
    ...
    it("should prevent claiming bond whith address other than the proposer address", async function () {
      await expect(
        this.BeneficiaryGovernance.connect(owner).claimBond(PROPOSALID)
    ...
    it("should prevent claiming bond for a proposal which has not passed.", async function () {
      ...
      await expect(
        this.BeneficiaryGovernance.connect(proposer1).claimBond(PROPOSALID)
    ...
    it("should be able to claim bond after a proposal passed.", async function () {
      ...
      expect(
        await this.BeneficiaryGovernance.connect(proposer1).claimBond(
          PROPOSALID
        )
      )
